...
contextBridge.exposeInMainWorld("viai", {
  ...
  background: {
      scanReportPage: (query: unknown) => ipcRenderer.invoke("background:scan-report-page", query) as Promise<unknown>,
      scanReport: (scanId: string) => ipcRenderer.invoke("background:scan-report", scanId) as Promise<unknown>,
      onScanReportUpdated: (listener: (update: unknown) => void) => { const handler = (_event: Electron.IpcRendererEvent, update: unknown) => listener(update); ipcRenderer.on("scan-report:updated", handler); return () => ipcRenderer.removeListener("scan-report:updated", handler); },
    snapshot: () => ipcRenderer.invoke("background:snapshot") as Promise<unknown>,
    historyPage: (query: { page?: number; pageSize?: number; search?: string; category?: "all" | "needs-investigation" | "monitoring" | "no-action" }) => ipcRenderer.invoke("background:history-page", query) as Promise<unknown>,
    dashboardSummary: () => ipcRenderer.invoke("background:dashboard-summary") as Promise<unknown>,
    assessmentTrend: (period: "24h" | "7d" | "30d") => ipcRenderer.invoke("background:assessment-trend", period) as Promise<unknown>,
    recentAssessments: (query: { limit?: number; search?: string; category?: "all" | "needs-investigation" | "monitoring" | "no-action" | "legacy" }) => ipcRenderer.invoke("background:recent-assessments", query) as Promise<unknown>,
    ...
    removeHistoryMatching: (query: { search?: string; category?: "all" | "needs-investigation" | "monitoring" | "no-action" }, excludedIds: string[]) => ipcRenderer.invoke("background:remove-history-matching", query, excludedIds) as Promise<unknown>,
  ...
  scans: {
    ...
    resume: () => ipcRenderer.invoke("scan:resume") as Promise<void>,
      continue: (scanId: string) => ipcRenderer.invoke("scan:continue", scanId) as Promise<void>,
