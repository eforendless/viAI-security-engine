...
export default function History() {
  ...
  return <motion.div {...pageMotion} className="page-stack"><div className="page-title split-title"><div><p className="eyebrow">LOCAL RECORDS</p><h2>Analysis history</h2><p>Full local reports, recommendations, trust evidence, and rule matches remain available for review.</p></div><Button onClick={exportJson} disabled={!results.length}><Download size={16} />Export JSON</Button></div><Panel className="history-panel"><div className="toolbar"><div className="history-filters"><label className="search-box"><Search size={17} /><span className="sr-only">Search analysis history</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search files and local events" /></label><label className="history-filter"><SlidersHorizontal size={16} /><span className="sr-only">Filter by risk level</span><select value={riskFilter} onChange={(event) => setRiskFilter(event.target.value as typeof riskFilter)} aria-label="Filter by risk level"><option value="all">All risk levels</option><option value="low">Low risk</option><option value="medium">Medium risk</option><option value="high">High risk</option></select></label></div><span>{results.length} record{results.length === 1 ? "" : "s"}</span></div>{results.length ? <div className="history-viewport" onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)}><div className="history-virtual-list" style={{ height: results.length * rowHeight }}>{visible.map((item, offset) => <div className="history-row" key={item.id} style={{ transform: `translateY(${(first + offset) * rowHeight}px)` }}><FileJson size={18} className="file-icon" /><div className="file-cell"><strong title={item.filePath}>{item.filePath?.split(/[\\/]/).pop() ?? item.detail}</strong><span>{new Date(item.occurredAt).toLocaleString()} - {item.recommendation ?? "Local event"} - trust {item.trust ?? 0}</span><span>{item.rules.length ? `Rules: ${item.rules.join(", ")}` : item.detail}</span></div><RiskBadge risk={item.risk} /><strong className="score-cell">{item.score}</strong><Link className="text-link" to={`/details/${item.id}`}>View details</Link></div>)}</div></div> : <div className="empty-state"><FileJson size={30} /><div><strong>No matching local records</strong><p>Completed analyses and realtime events will appear here.</p></div></div>}</Panel></motion.div>;
}
