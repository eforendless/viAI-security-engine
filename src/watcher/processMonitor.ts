...
export class ProcessMonitor {
  ...
  start(policy: ProcessMonitorPolicy, intervalMilliseconds = 5_000): void {
    this.policy = policy;
  ...
  private async refresh(initial: boolean): Promise<void> {
    ...
    for (const entry of processes) {
      if (typeof entry.ProcessId !== "number" || this.knownProcessIds.has(entry.ProcessId) || initial || !shouldMonitorProcess(entry, this.policy)) continue;
...
export function shouldMonitorProcess(processInfo: WindowsProcess, policy: ProcessMonitorPolicy): boolean {
  ...
  return policy.monitorNewProcesses || policy.monitorChildProcesses || selectedHost || suspiciousCommand;
}
