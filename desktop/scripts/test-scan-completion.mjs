...
assert.equal(state.scan.status, "completed");
assert.equal(state.scan.active, false);
assert.equal(state.scan.id, "completed-scan");
assert.equal(state.scan.completed, 842_531);
assert.equal(state.scan.total, 842_531);
assert.equal(state.scan.elapsedMs, 1_122_000);
assert.equal(state.scan.estimatedRemainingMs, undefined);
