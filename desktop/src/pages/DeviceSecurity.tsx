...
interface DeviceScan { id: string; deviceId: string; startedAt: string; finishedAt?: string; filesScanned: number; threatsFound: number; status: string; findings: Array<{ filePath: string; riskScore: number; recommendation: string; evidence: string[] }>; }
...
function UsbActivity({ devices, scans }: { devices: Device[]; scans: DeviceScan[] }) {
  return <div className="page-stack"><Panel><div className="panel-heading"><div><h3>Removable storage activity</h3><p>Drive details and scan lifecycle records for connected removable media.</p></div></div><div className="storage-table">{devices.length ? devices.map((device) => <div className="storage-row" key={device.id}><HardDrive size={20} /><div><strong>{device.friendlyName}</strong><span>{device.mountPoint ?? "Mount path unavailable"} {device.fileSystem ? `- ${device.fileSystem}` : ""}</span></div><span>{device.capacity ? formatBytes(device.capacity) : "Capacity unavailable"}</span><Status status={device.status} /></div>) : <Empty icon={HardDrive} title="No removable storage connected" detail="USB drives, SD cards, and external storage will be monitored here." />}</div></Panel><Panel><div className="panel-heading"><div><h3>Scan activity</h3><p>Static analysis reports are attached to completed device scans.</p></div></div><div className="device-event-list">{scans.length ? scans.map((scan) => <div key={scan.id} className="device-scan-result"><div className="scan-row"><Usb size={18} /><div><strong>{scan.status === "running" ? "Removable media scan in progress" : scan.status === "failed" ? "Removable media scan unavailable" : "Removable media scan complete"}</strong><span>{scan.filesScanned} eligible files scanned - {scan.threatsFound} requiring investigation</span></div><span>{formatTime(scan.finishedAt ?? scan.startedAt)}</span></div>{scan.findings.filter((finding) => finding.riskScore > 0).slice(0, 5).map((finding) => <div className="device-scan-finding" key={finding.filePath}><strong>{finding.filePath.split(/[\\/]/).pop()}</strong><span>Risk {finding.riskScore} - {finding.recommendation}</span><small>{finding.evidence[0] ?? "No additional evidence"}</small></div>)}</div>) : <Empty icon={ShieldCheck} title="No device scans yet" detail="Removable storage scans begin automatically after connection." />}</div></Panel></div>;
}
